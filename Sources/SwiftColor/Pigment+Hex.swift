...
extension Pigment {
    ...
    static func hex8(
        ...
        let red = Double((hex & 0xFF000000) >> 24) / 255.0
        let green = Double((hex & 0x00FF0000) >> 16) / 255.0
        let blue = Double((hex & 0x0000FF00) >> 8) / 255.0
        let alpha = Double((hex & 0x000000FF) >> 0) / 255.0
        return (red, green, blue, alpha)
    }
